                    ...
                    <div class="project-card group overflow-hidden rounded-lg sm:rounded-xl md:rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 md:hover:-translate-y-2 scroll-reveal-item"
                        ...
                        <div class="p-4 sm:p-5 md:p-6 bg-white scroll-reveal-item">
                            <div class="flex items-center justify-between mb-2 sm:mb-3">
                                <span
                                    class="text-xs {{ cetak($project->badge_color) }} px-2 py-1 rounded-full font-medium">
                                    {{ cetak($project->kategori->name) }}
                                </span>
                                <div class="flex items-center text-yellow-400">
                                    <!-- Star Ratings -->
                                    <svg class="w-3 h-3 sm:w-4 sm:h-4 fill-current" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 20 20">
                                        <path
                            ...
                            <div class="mb-3 sm:mb-4">
                                <span class="text-base sm:text-lg md:text-xl font-bold text-gray-900">Rp
                                    {{ number_format(cetak($project->price)), 0, ',', '.' }}</span>
                                @if ($project->original_price)
                                    <span class="text-xs sm:text-sm text-gray-500 line-through ml-2">Rp
                                        {{ number_format(cetak($project->original_price)), 0, ',', '.' }}</span>
                                @endif
                            </div>
                            ...
                            @if ($project->funding_goal)
                                <div class="mb-3 sm:mb-4">
                                    <div class="flex justify-between text-xs text-gray-500 mb-1">
                                        <span>{{ ceil(($project->funds_raised / $project->funding_goal) * 100) }}%
                                            funded</span>
                                        <span>Rp {{ number_format($project->funds_raised) }} of Rp
                                            {{ number_format($project->funding_goal) }}</span>
                                    </div>
                                    <div class="w-full bg-gray-200 rounded-full h-2">
                                        <div class="bg-blue-500 h-2 rounded-full"
                                            style="width: {{ ($project->funds_raised / $project->funding_goal) * 100 }}%">
                                        </div>
                                    </div>
                                </div>
                            @endif
        ...
        function initBuyNowButtons() {
            document.querySelectorAll('.buy-now-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    document.getElementById('project-price-display').textContent = 'Rp ' + parseInt(
